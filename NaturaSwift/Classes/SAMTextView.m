...
- (CGRect)placeholderRectForBounds:(CGRect)bounds {
    ...
    CGRect rect55 = UIEdgeInsetsInsetRect(bounds, self.contentInset);

    CGRect rect66 = UIEdgeInsetsInsetRect(rect55, self.textContainerInset);

    UITextView *textView = [[UITextView alloc] init];
    
    NSMutableAttributedString *strAA = [self.attributedPlaceholder mutableCopy];
    
    [strAA.mutableString setString: [self placeholder] ];
    
    [textView setAttributedText:  strAA ];

    CGSize size = [textView sizeThatFits:CGSizeMake(rect66.size.width, FLT_MAX)];
    

    CGRect rect1 = UIEdgeInsetsInsetRect(bounds, textView.contentInset);

    if (size.height < 35)
    {
        rect1 = UIEdgeInsetsInsetRect(rect1, self.textContainerInset);
    }
    
    CGFloat padding = self.textContainer.lineFragmentPadding;
    rect1.origin.x += padding;
    rect1.size.width -= padding * 2.0f;
