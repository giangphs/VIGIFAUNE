    ...
    if ([[self delegate] respondsToSelector:@selector(textFieldShouldSelect:)]) {
        if ([[self delegate] textFieldShouldSelect:self]) {
            if ([self applyFilterWithSearchQuery:self.text].count > 0) {
                self.text = [[[self applyFilterWithSearchQuery:self.text] objectAtIndex:0] objectForKey:@"email"];
                ...
                if ([[self delegate] respondsToSelector:@selector(textField:didEndEditingWithSelection:)]) {
                    [[self delegate] textField:self didEndEditingWithSelection:[NSDictionary dictionaryWithObjectsAndKeys:self.text,@"email",@"",@"pwd", nil]];
                }
                else{
                    NSLog(@"<MPGTextField> WARNING: You have not implemented a method from MPGTextFieldDelegate that is called back when the user selects a search suggestion.");
                }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[cell textLabel] setText:[dataForRowAtIndexPath objectForKey:@"email"]];
    
    [[cell textLabel] setTextColor: UICOLOR_BLUE];
...
{
    self.text = [[[self applyFilterWithSearchQuery:self.text] objectAtIndex:indexPath.row] objectForKey:@"email"];
...
{
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"email BEGINSWITH[cd] %@", filter];
...
- (void)provideSuggestions
{
    ...
    if (tableViewController.tableView.superview == nil && [[self applyFilterWithSearchQuery:self.text] count] > 0) {
        ...
        tableViewController.tableView.layer.cornerRadius = 0;
        ...
        if (self.popoverSize.size.height == 0.0) {
            ...
            frameForPresentation.origin.x = 26;
            frameForPresentation.origin.y += self.frame.size.height;
            frameForPresentation.size.height = 90;
            frameForPresentation.size.width = frameForPresentation.size.width + 2*(40 - 26) + 2;
